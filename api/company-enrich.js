...
// Checks masothue + tavily coverage — if not yet enriched (or stale),
...
export default async function handler(req, res) {
  ...
  // Check masothue coverage — skip if already enriched recently
  const { data: cov } = await supabase
    .from('coverage')
    .select('status, checked_at')
    .eq('entity_id', cr.entity_id)
    .eq('source_type', 'masothue')
    .maybeSingle();

  if (cov?.status === 'found') {
    const daysSince = (Date.now() - new Date(cov.checked_at).getTime()) / 86400000;
    if (daysSince < 30) {
