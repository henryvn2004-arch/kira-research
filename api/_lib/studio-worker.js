...
// Web-search budget for the research turn.
const SEARCH_MAX_USES     = 18;
...
// ===============================================================
// STAGE 4 — Web search (Anthropic's web_search tool)
// ===============================================================
async function stage4Research({ parsed, plan }) {
  const c = client();
  const system = `You are a research analyst at KIRA Research preparing source material for a market report.

Run web searches to gather the data and citations the writer will need. Cover both ENGLISH and the country's local-language business press (${parsed.local_language_name || 'English'}, code ${parsed.local_language_code || 'en'}) — search in BOTH languages for tier-1 KIRA markets (vi/id/th/ja/ko) or whenever the country is non-English-dominant.

For each section in the plan that has needs_search=true:
1. Fire 2-4 searches in English.
2. Fire 1-2 searches in the local language for the country (translate the query terms; keep proper nouns + acronyms in original form).
3. Capture: source name, year, URL, the key quantitative claim or qualitative finding.

After all searches, output ONE structured JSON object summarising findings. NO prose preamble. JSON shape:

{
  "findings": [
    {
      "section_title": "<matching the plan>",
...
Begin researching. After you've exhausted your search budget, return the JSON findings object.`;
  ...
  // Extract any text blocks (the final JSON) from the response.
  const text = textFromMessage(msg);
  let findings;
  try {
    findings = parseJsonLoose(text);
  } catch {
    // If model didn't emit clean JSON (rare), fall back to a thin
