...
--
-- Reran-against-legacy note: an older `insights` table existed on the
-- platform-era schema. CREATE TABLE below is greenfield-only; the ALTER
-- block right after backfills missing columns into a legacy table so this
-- file is safe to run either way.
...
-- Long string literals use dollar quoting ($tag$...$tag$) so apostrophes,
-- em-dashes, and HTML attributes pass through verbatim — robust against
-- smart-quote autocorrect and Unicode mangling in SQL paste pipelines.
insert into public.insight_translations (insight_id, locale, title, excerpt, lede, body, read_time, status, published_at)
select
  i.id, 'en',
  $title$How Vietnam's super-apps quietly rewrote SME lending in 18 months.$title$,
  $excerpt$Three operators now originate more SME credit than any commercial bank under VND 500M ticket size. The shift isn't about AI underwriting — it's about who controls the cash-flow data.$excerpt$,
  $lede$Three super-app operators now originate more SME credit under VND 500M ticket size than any commercial bank in Vietnam. The shift didn't happen because AI underwriting finally worked — it happened because someone else now owns the cash-flow data.$lede$,
  $body$<p>The conventional story is that fintech disrupted lending by replacing human credit officers with machine-learning models. That framing matches the press releases, but it doesn't match what we're seeing in the field. The Vietnamese case is a useful counter-example because the technology under the hood is — frankly — not very interesting. What's interesting is who has the data.</p><p>Eighteen months ago, the leading domestic super-apps started bundling working-capital credit products into their merchant-payments tools. The product looked nearly identical to commercial-bank SME lines on paper: rolling line, monthly review, similar pricing. The difference was the underwriting input: instead of relying on tax returns and bank statements (the inputs banks have always used), the super-apps underwrote against <strong>14 months of in-product cash flow</strong> — every settlement, every refund, every chargeback, every dispute, captured natively.</p><blockquote>The competitive moat isn't the model. It's the data the model is fed.</blockquote><h2>What we've heard in the field</h2><p>We spoke with merchants across HCMC and Hanoi who use multiple payment rails. The consistent pattern: <strong>approval rates 2–3× higher</strong> on super-app lines vs their primary bank, despite the bank holding deeper relationship history. Decision turnaround was the second consistent signal — 24–72 hours on super-app lines vs 2–4 weeks for the equivalent bank product.</p><p>Both numbers point in the same direction: the super-apps aren't taking on dramatically worse credit; they're seeing the borrower more accurately. The data they hold is higher-fidelity than what the SME would normally submit to a bank, and it's live. Once you have a clean read on a borrower's cash flow, the underwriting model becomes almost commodity.</p><h2>Why this matters for incumbents</h2><p>The standard incumbent response — "we'll just build the AI model too" — doesn't solve the actual problem. The problem isn't model quality. It's data possession. Banks looking at this market structure now have two real options:</p><ul><li>Acquire the data layer (partnerships with payment platforms; data-sharing rights)</li><li>Compete on segments where the super-apps don't have the data yet (cross-border, project finance, secured)</li></ul><p>The thing that doesn't work is pretending the gap is technological. We've seen at least three banks make that mistake in the last 12 months and shed market share through it.</p><h3>Methodology note</h3><p style="font-size:14px;color:var(--muted);font-style:italic;">This field note is drawn from analyst interviews conducted across Vietnamese SME operators between February and April 2026, supplemented by our 2026 Vietnam Fintech Landscape report. Full sourcing, charts, and operator-level cost analysis are in the paid report.</p>$body$,
  '6 min read',
  'published',
  now() - interval '8 days'
...
-- until full body is added. Dollar-quoted strings ($$...$$) avoid apostrophe
