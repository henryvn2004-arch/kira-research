...
description: "Phase Q.2 — insight cron extracts sections from each published report's en.html and gens trilingual Insight articles for AEO/GEO. NOTE: target is now 4 insights/report (3 prose + 1 key-data-metrics), 12 translation rows, up to 6 fires — not the 3/9/3 described below."
metadata: 
  node_type: memory
  type: project
  originSessionId: e151b86a-def6-4566-bcc2-20da087b0296
  modified: 2026-07-29T05:02:54.974Z
---

## ⚠️ Đính chính 2026-07-29 — target là 4 insight/report, không phải 3

Mọi con số "3 insight / 9 row / 3 fire" phía dưới đã **superseded**. Nguồn đúng là
`skills/kira-research-report/prompts/insight_runner.md:11`:

> "For every published report, produce **4 insights × 3 languages = 12
> insight_translations rows** in up to 6 fires"

Insight thứ 4 có `slug_key = "key-data-metrics"` (slug
`<industry>-<country>-key-data-metrics-<year>`) — 2–3 bảng `class="kira-data-table"`,
thuần số liệu không văn xuôi, để AI engine trích dẫn + làm infographic.

**Hệ quả quan trọng:** một report có 4 insight là **đúng**, không phải dư. Đừng
xoá bớt về 3. Khi soi trùng lặp thật (xem [[feedback_insight_fires_no_claim_race]])
thì ngưỡng là **> 4**, và phải so bằng `slug`, không phải bằng số lượng thuần.
Kiểm 2026-07-29: 26 report có đúng 4 — bình thường; riêng
`vietnam-logistics-2026` có **6** — bất thường, chưa điều tra.

Cadence "4 fire/ngày" phía dưới cũng cũ: Phase Q.7 (2026-07-29) gộp còn **1 fire
insight/ngày** lúc 03:00 trong routine `kira` — xem [[project_batch_cron_system]].
